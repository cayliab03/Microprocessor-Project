...
void spinclockwise(){
    ...
    for(int i=0; i<512*12/360; i++){
        ...
        SysTick_Wait10ms(5);
		// Blink D2 LED every 12 iterations
		if(i%12==0&&i!=0){
				GPIO_PORTN_DATA_R = 0b00000001; // Turn on D2
				SysTick_Wait10ms(10);
				GPIO_PORTN_DATA_R = 0b00000000; // Turn off D2
        }
...
int main(void) {
  ...
  // Blink D1 once at startup
  GPIO_PORTN_DATA_R = 0b00000010; // Turn on D1
  SysTick_Wait10ms(10);
  GPIO_PORTN_DATA_R = 0b00000000; // Turn off D1	
  ...
  while(1) {
		...
		if((GPIO_PORTM_DATA_R&0b00000001)==1){ // If button is pressed, PM0
	

		// Get the Distance measurement 30 times
		int angle = 0;
		for(int j = 0; j < 30; j++) {
	...
	if (x%2==0){
	  ...
	  status = VL53L1X_ClearInterrupt(dev); // clear interrupt has to be called to enable next interrupt
		// compute the y and z coordinates of a point in a 3D space using polar coordinates
		int y = Distance*(sin(angle*3.14159265359/180)); 
    int z = Distance*(cos(angle*3.14159265359/180));
